# train_v1.4.2.py - CRNN + CTC OCR 학습 스크립트
...
best_path = os.path.join(bestmodel_dir, "ocrBestModel_142.pth")
...
def custom_collate_fn(batch):
    # batch: list of (image_tensor, text_str)
    images, texts = zip(*batch)
    # 이미지들은 shape이 같은 텐서로 쌓아서 새로운, resizable storage에 담습니다.
...
if __name__ == "__main__":
    ...
    transform = transforms.Compose([
        # ① 회전/왜곡 등 Augmentation
        transforms.RandomRotation(15, expand=False, fill=255),
        transforms.RandomAffine(degrees=1, translate=(0.01, 0.01), fill=255),
        # 원근 왜곡
