...
# train_v1_5_3_mini_kor_hs512.py - 미니 한글 데이터셋, hidden_size=512, OneCycleLR 적용
...
MINI_KOR_CHARSET = CHOSUNG_LIST + JOONGSUNG_LIST + list(NUMBER_CHARS)
# --- 문자셋 정의 끝 ---
...
best_path = bestmodel_dir / "ocrBestModel_153_mini_kor_hs512.pth" # 새 모델 경로
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="crnn-ocr-mini-kor-hs512", # 새 프로젝트 이름
        name="train_v1.5.3_mini_kor_hs512",
    ...
    train_dataset = OCRDataset(
        image_dir=str(ocr_dir / 'data/virtual_data_mini_kor/images'),
        label_dir=str(ocr_dir / 'data/virtual_data_mini_kor/labels'),
        transform=transform
    )
    val_dataset = OCRDataset(
        image_dir=str(ocr_dir / 'data/virtual_val_data_mini_kor/images'),
        label_dir=str(ocr_dir / 'data/virtual_val_data_mini_kor/labels'),
        transform=transform
    )
    ...
    encoder = LabelEncoder(charset=MINI_KOR_CHARSET)
    ...
    try:
        for epoch in range(config.epochs):
            ...
            # ReduceLROnPlateau와 달리 OneCycleLR은 val_loss를 사용하지 않음
        ...
        torch.save(model.state_dict(), ocr_dir / 'checkpoints/last_interrupted_v1.5.3.pth')
    ...
    df.to_csv(log_dir / 'train_log_v1.5.3.csv', index=False)
    print(f"Training log saved: {log_dir / 'train_log_v1.5.3.csv'}")
