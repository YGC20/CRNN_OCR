...
# train_v1_5_2_mini_kor_test.py - 미니 한글 데이터셋으로 CRNN + CTC OCR 학습 스크립트
...
best_path = bestmodel_dir / "ocrBestModel_152_mini_kor.pth" # 새 모델 경로
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="crnn-ocr-mini-kor-test", # 새 프로젝트 이름
        name="train_v1.5.2_mini_kor",
        dir=str(ocr_dir / "wandb_logs"),
        config={
            "batch_size": 128,
            "lr": 0.001,
            "epochs": 30, # 테스트용으로 에포크 줄임
            "hidden_size": 256,
    ...
    try:
        ...
        torch.save(model.state_dict(), ocr_dir / 'checkpoints/last_interrupted_v1.5.2.pth')
    ...
    df.to_csv(log_dir / 'train_log_v1.5.2.csv', index=False)
    print(f"Training log saved: {log_dir / 'train_log_v1.5.2.csv'}")
