...
# --- 설정 ---
# (기존 create_virtual_data.py에서 영문자만 제외)
OUTPUT_DIR = os.path.join("ocr", "data", "virtual_data_kor")
NUM_IMAGES = 115000  # 전체 데이터셋과 동일한 규모
IMAGE_WIDTH = 200
IMAGE_HEIGHT = 50
...
def generate_image(char, font):
    """지정된 문자로 이미지를 생성합니다."""
    image = Image.new('L', (IMAGE_WIDTH, IMAGE_HEIGHT), color=255)  # 흰색 배경
    ...
    draw.text((x, y), char, fill=0, font=font)  # 검은색 글자
    return image
...
def main():
    ...
    for i in range(NUM_IMAGES):
        # 균등 분배를 위해 KOR_CHARSET에서 문자를 순환하며 선택
    ...
    print(f"Successfully generated {NUM_IMAGES} images and labels.txt in {OUTPUT_DIR}")
