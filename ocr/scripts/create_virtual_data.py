...
NUM_VARIATIONS_PER_CHAR = 500  # 문자 당 생성할 이미지 수 (대폭 상향)

# 경로 설정
SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parent.parent.parent
OUTPUT_DIR = PROJECT_ROOT / "ocr/data/virtual_data"
...
FONT_PATHS = [
    ...
    "C:/Windows/Fonts/H2GTRE.TTF",      # 나눔고딕 (설치된 경우)
    "C:/Windows/Fonts/H2GTRM.TTF",      # 나눔고딕 (설치된 경우)
]
...
def generate_data():
    ...
    for char in TOTAL_CHARS:
        ...
        for i in range(NUM_VARIATIONS_PER_CHAR):
            ...
            if random.random() < 0.5:
                augmented_img = change_brightness_contrast(augmented_img)
            if random.random() < 0.5:
                augmented_img = apply_blur(augmented_img, strength=random.random())
            if random.random() < 0.5:
                augmented_img = add_noise(augmented_img, strength=random.random())

            # 4. 파일 저장 (오류 처리 추가)
            safe_char = char if char.isalnum() else f"c{ord(char)}"
            filename = f"virtual_{safe_char}_{i:03d}"
            img_path = IMAGE_DIR / f"{filename}.png"
            lbl_path = LABEL_DIR / f"{filename}.txt"

            try:
                # 1. 이미지를 메모리 버퍼로 인코딩 (더 안정적인 방식)
                result, buffer = cv2.imencode('.png', augmented_img)
                if not result:
                    print(f"\n[오류] cv2.imencode()가 이미지 인코딩을 실패했습니다. 문자: '{char}'")
                    continue

                # 2. Python의 open()을 사용하여 파일에 쓰기
    ...
    print("\n" + "-"*50)
    print(f"✅ 총 {total_count}개의 가상 데이터 생성이 완료되었습니다.")
    print("-"*50)
